...
class RequestController extends Controller
{
    ...
    //     $itemReq = UserRequest::findOrFail($id);

    //     $itemReq->rating = $request->rating;
    //     $itemReq->update();

    //     $requestedUser = User::findOrFail($itemReq->id_requested); // cari tukang
    //     $all_request_this_user = UserRequest::where([['id_requested', $requestedUser->id], ['status', 'Finished']])->get(); //cari rating tiap request
        
    //     $rating = 3;
    //     $no = 0;

    //     foreach($all_request_this_user as $res){
    ...
    public function updateRating($requestedUser) {
        $initialRating = 3; // Initial rating value
    
        $finishedRequests = UserRequest::where([
            ['id_requested', $requestedUser->id],
            ['status', 'Finished'],
        ])->get();
        ...
        $count = count($finishedRequests) + 1; // Including the initial rating
    
        $ratingAvg = $totalRating / $count;
    
        $requestedUser->rating = $ratingAvg;
        $requestedUser->save();
    }
    ...
    public function rating($id, Request $request) {
        ...
        // Fetch the requested user only once
        $requestedUser = User::findOrFail($itemReq->id_requested);
    
        // Update the rating for the requested user
        $this->updateRating($requestedUser);
